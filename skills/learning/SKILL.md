...
- **Folders:** `/Concepts` `/Methods` `/Sources` `/Maps` `/Sessions`
- **Templates:** `/Templates/concept.md`, `/Templates/session.md`
...
Read the named source material for *today only* (narrow, not the whole subject folder).
Then read the vault:
...
3. **Set frontmatter** per the status ladder, including `tags`.
...
last_reviewed: YYYY-MM-DD # any contact, including a re-read
last_unaided: YYYY-MM-DD  # last cold, UNAIDED pass — the only thing that earns/renews status
review_due: YYYY-MM-DD
sources: []
sessions: []
prerequisites: []        # foundations this note rests on — INCLUDING unresolved [[links]] to not-yet-studied concepts
related: []              # cross-subject links and downstream topics
anki:                    # optional: deck/tag if this concept leans on a specific atom set — only when it helps
exam_weight:
```
*(Add `last_unaided:` and, if used, `anki:` to `/Templates/concept.md`.)*
